...
public class PermissionAndNetUtils {
    ...
    public static void onRequestPermissionResult(Context context, String permission, int[] grantResults, PermissionCheckCallBack callback) {
        if (PermissionAndNetUtils.isPermissionRequestSuccess(grantResults)) {
            callback.onHasPermission();
        } else {
            if (PermissionAndNetUtils.judgePermission(context, permission)) {
                callback.onUserHasAlreadyTurnedDown(permission);
            } else {
                callback.onUserHasAlreadyTurnedDownAndDontAsk(permission);
            }
    ...
    public static void StatusTitleColor(Context context){
        StatusBarUtil.setColor((Activity) context, Color.parseColor("#E71442"),0);
    }
}
